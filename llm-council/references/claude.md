...
The supplied definitions use `model: inherit` and `tools: []`, with no persistent memory or preloaded skills. An explicitly empty tool list is documented to launch without tools. This is a supplied-input design: the coordinator gathers evidence and writes artifacts; workers return analysis. Verify the effective tool/model settings in the running version. [Empty-tool behavior](https://code.claude.com/docs/en/errors#agent-would-be-spawned-with-zero-tools)

Foreground or background execution is acceptable. Collect five final advisor answers before anonymizing, five final reviews before starting the chair, and the chair's final verdict before writing artifacts. A task identifier, acknowledgement, or progress update is not a completed result. Use isolated batches when capacity requires them; do not bypass limits through nested delegation or agent teams.
