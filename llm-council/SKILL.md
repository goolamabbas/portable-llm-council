---
name: llm-council
description: "Run a five-advisor council with blind peer review for consequential decisions. Use when the user requests a council or a multi-perspective review of a consequential decision; skip quick critiques and routine factual or writing tasks."
---
...
Frame a neutral decision brief from the user's question and relevant available context: objectives, options, constraints, stakes, facts, and unknowns. Gather context proportionately; ask only when a missing detail would materially change the decision and cannot be resolved from available context or an explicit conditional assumption. Keep evidence distinguishable from assumptions.

Read [subagent assignments](references/subagents.md) for the roles and their inputs. Use the current harness's available subagent tools. When dispatch, isolation, capacity, or role selection needs clarification, read only the matching adapter: [Codex](references/codex.md), [Cursor](references/cursor.md), [Command Code](references/commandcode.md), [Grok Build](references/grok-build.md), [OpenCode](references/opencode.md), [Factory Droid](references/factory-droid.md), or [Claude Code](references/claude.md). Identify the harness from the active environment, not the selected model. Other harnesses require capability verification before use. Reuse capability checks established in the current session unless the runtime, configuration, or dispatch mode changes. Check each reviewer's supplied input packet; record unobservable settings as unverified and do not claim unestablished isolation.
...
Save `council-report-{timestamp}.html` and `council-transcript-{timestamp}.md` in the requested directory, defaulting to the current project. Use matching, filesystem-safe timestamps and avoid overwriting past sessions. If recovery is impossible, save both artifacts with a prominent incomplete status, the returned material, and the blocker; do not invent a chair verdict or present the council as complete.
